...
export function CasamentoCertificateBuilder({ igrejaNome, logoPath, logoUrl }: BuilderProps) {
  ...
  const {
    ...
    setCampos,
    handleAddToCart,
    getSummary: (row, current) => {
      const noivo = (typeof row.nomeNoivo === "string" && row.nomeNoivo.trim()) || current.nomeNoivo || "";
      const noiva = (typeof row.nomeNoiva === "string" && row.nomeNoiva.trim()) || current.nomeNoiva || "";
      const summary = [noivo, noiva].filter(Boolean).join(" & ");
