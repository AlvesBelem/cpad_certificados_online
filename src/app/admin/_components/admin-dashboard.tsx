...
export function AdminDashboard({ data, adminName }: AdminDashboardProps) {
  ...
  function handleUserRoleChange(updated: DashboardUser) {
    ...
    setEmployees((previous) => {
      const filtered = previous.filter((user) => user.id !== updated.id);
      if (updated.role === UserRole.EMPLOYEE) {
        return sortUsers([...filtered, updated]);
      }
      return filtered;
    });
                  ...
                  <TableRow key={user.id}>
                    ...
                    <TableCell>
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value={UserRole.USER}>Cliente</SelectItem>
                          <SelectItem value={UserRole.EMPLOYEE}>Funcionario</SelectItem>
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => (
            ...
            >
              ...
              <span className="text-base font-semibold">{currencyFormatter.format(item.totalRevenue)}</span>
            </div>
          ))
        )}
      </CardContent>
